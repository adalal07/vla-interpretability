...
def export_per_episode_csv(info: dict, output_dir: Path, condition_label: str | None = None) -> None:
    ...
    if "per_episode" in info:
        ...
        for task_info in info["per_task"]:
            ...
            for i in range(n_eps):
                row = {
                    ...
                    "task_id": task_id,
                    "episode_ix": i,
                    "sum_reward": metrics["sum_rewards"][i],
        ...
        logging.warning("No per-episode data found for CSV export")
        return
    ...
    logging.info(f"Exported {len(rows)} episodes to {csv_path}")
...
def eval_main(cfg: EvalPipelineConfig):
    ...
    # Add condition label to info if specified
    if cfg.eval.condition_label:
        info["condition_label"] = cfg.eval.condition_label
